...
from .models import (
    ...
    Medication, Ward, Bed, BirthRecord, MortalityRecord, CancerRegistryReport, LabTest, VisitorEntryExit, Billing, Service, Visit, Insurance
)
...
class InsuranceForm(forms.ModelForm):
    class Meta:
        model = Insurance
        fields = ['company_name', 'policy_number', 'expiry_date']
        widgets = {
...
class PatientRegistrationForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['has_insurance'].widget.attrs['class'] = 'form-check-input'
...
class PatientForm(forms.ModelForm):
    """
    A form for updating an existing Patient profile.
    This form handles all patient data fields, as the Patient model no longer
    links to a User model for personal information.
    """
    class Meta:
        model = Patient
        fields = [
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Add Bootstrap form-control class and placeholders to all fields
        for field_name, field in self.fields.items():
            ...
            if field_name == 'first_name':
                ...
                field.widget.attrs['placeholder'] = 'Address'
        ...
        # Add a read-only field for patient_id at the top for display
        if self.instance and self.instance.patient_id:
            ...
            # Reorder fields to place display_patient_id first
            new_order = ['display_patient_id'] + list(self.fields.keys())[:-1]
...
class AppointmentForm(forms.ModelForm):
    class Meta:
        model = Appointment
        fields = ['patient', 'doctor', 'appointment_date', 'reason_for_visit', 'status']
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.user:
            if self.user.user_type == 'patient':
                ...
                # Patient should not see or change status
                ...
                # Doctor should not see or change status
                self.fields['status'].widget = forms.HiddenInput()
            elif self.user.user_type in ['admin', 'receptionist']:
                self.fields['patient'].queryset = Patient.objects.all().order_by('first_name')
                self.fields['doctor'].queryset = Doctor.objects.all().order_by('user__first_name')
                # These users should have full access to the status field
...
class VisitorEntryExitForm(forms.ModelForm):
    class Meta:
        model = VisitorEntryExit
        fields = [
            'event_type', 'visitor_name', 'visitor_contact',
            'reason_for_visit', 'related_patient', 'recorded_by'
        ]
        widgets = {
            'recorded_by': forms.HiddenInput(), # This will be set in the view based on request.user
        }
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['related_patient'].queryset = Patient.objects.select_related('user').all().order_by('user__first_name')
        self.fields['related_patient'].label_from_instance = lambda obj: f"{obj.user.get_full_name()} ({obj.patient_id})"
        # Make visitor_name and visitor_contact not required if related_patient is selected
        self.fields['visitor_name'].required = False
        self.fields['visitor_contact'].required = False

from django import forms
from .models import Billing, Patient, Encounter

# In your forms.py

from django import forms
from .models import Billing, Encounter, Patient

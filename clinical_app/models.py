...
class Patient(models.Model):
    ...
    # 🔹 NEW insurance-related fields
    has_insurance = models.BooleanField(default=False)
    insurance = models.ForeignKey(
        Insurance, on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    def __str__(self):
        return f"{self.full_name} ({'Insurance' if self.has_insurance else 'Cash'})"

class Visit(models.Model):
    """
    Represents an outpatient visit.
    Each visit may have multiple encounters 
    (consultations, vitals, lab requests, pharmacy, etc.)
    """
    patient = models.ForeignKey(
        Patient, on_delete=models.CASCADE, related_name="visits"
    )
    doctor = models.ForeignKey(
        "Doctor", on_delete=models.SET_NULL, null=True, blank=True, related_name="visits"
    )
    ...
    def __str__(self):
        return f"Visit for {self.patient.full_name} on {self.visit_date.strftime('%Y-%m-%d')}"
...
class Encounter(models.Model):
    ...
    """
    visit = models.ForeignKey("Visit", on_delete=models.CASCADE, related_name="encounters", null=True, blank=True)     
    ...
    encounter_type_choices = (
        ('consultation', 'Consultation'),         # General doctor/nurse consultation
        ('follow_up', 'Follow-up Visit'),        # Returning after initial consultation
        ('emergency', 'Emergency'),              # Urgent outpatient care
        ('telemedicine', 'Telemedicine'),        # Remote video/phone consultation
        ('vitals', 'Vitals Check'),              # Temperature, BP, weight, etc.
        ('lab', 'Laboratory'),                   # Lab test encounter
        ('pharmacy', 'Pharmacy'),                # Drug dispensing or refill
        ('imaging', 'Imaging / Radiology'),      # X-ray, ultrasound, etc.
        ('procedure', 'Minor Procedure'),        # e.g., dressing, wound care
        ('counseling', 'Counseling / Education'),# Nutrition, mental health, etc.
        ('triage', 'Triage'),                    # Initial nurse screening before doctor
    )
...
class VitalSign(models.Model):
    patient = models.ForeignKey(Patient, on_delete=models.CASCADE, related_name='vital_signs', null=True, blank=True)
    encounter = models.ForeignKey(Encounter, on_delete=models.CASCADE, related_name='vital_signs', null=True, blank=True)
...
class Service(models.Model):
    """
    Defines the services offered by the hospital.
    """
    SERVICE_CATEGORIES = [
    ...
    name = models.CharField(max_length=255, unique=True)
    category = models.CharField(max_length=50, choices=SERVICE_CATEGORIES, default='other')
    description = models.TextField(blank=True, null=True)
    price = models.DecimalField(max_digits=10, decimal_places=2, help_text="Default price for the service")
    is_active = models.BooleanField(default=True)
...
class VisitorEntryExit(models.Model):
    """
    Represents an entry or exit event for a *general visitor* or a registered patient.
    """
    VISITOR_TYPE_CHOICES = [
        ('entry', 'Entry'),
        ('exit', 'Exit'),
    ]
    event_type = models.CharField(max_length=5, choices=VISITOR_TYPE_CHOICES)
    timestamp = models.DateTimeField(default=timezone.now)

    # These fields are for general visitors (delivery, relative, etc.)
    visitor_name = models.CharField(max_length=255, blank=True, null=True, help_text="Name of the visitor (if not a registered patient)")
    visitor_contact = models.CharField(max_length=100, blank=True, null=True, help_text="Contact info (e.g., phone number or email) for general visitor")
    reason_for_visit = models.CharField(max_length=255, blank=True, null=True, help_text="e.g., Visiting Patient X, Delivery, Inquiry")
    notes = models.CharField(max_length=255, blank=True, null=True)

    # Link to a Patient if the visitor IS a registered patient checking in/out
    related_patient = models.ForeignKey(Patient, on_delete=models.SET_NULL,
                                        null=True, blank=True, related_name='visitor_events',
                                        help_text="Link to patient record if this visitor is a registered patient")

    # Link to the User who recorded this entry/exit (e.g., Receptionist)
    recorded_by = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name='recorded_visitor_events',
        limit_choices_to={'user_type__in': ['admin', 'receptionist', 'nurse', 'doctor']}, # Only staff can record
        help_text="User who recorded this entry/exit event"
    )

    def __str__(self):
        # The related_patient field now points to a Patient model without a user field.
        # So we can use the full_name property directly.
        if self.related_patient:
            return f"{self.event_type.capitalize()} for Patient {self.related_patient.full_name} ({self.related_patient.patient_id}) at {self.timestamp.strftime('%Y-%m-%d %H:%M:%S')}"
        else:
            return f"{self.event_type.capitalize()} for General Visitor {self.visitor_name or 'Anonymous'} at {self.timestamp.strftime('%Y-%m-%d %H:%M:%S')}"
